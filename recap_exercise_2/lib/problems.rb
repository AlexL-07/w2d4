...
def least_common_multiple(num_1, num_2)
    ...
    (1..max).each do |f1|
        (1..max).each do |f2|
            return max * f1 if max * f1 == min * f2
        end
    end
    ...
    def bubble_sort(&prc)
        ...
        while !sorted
            ...
            (0...self.length - 1).each do |i|
                if prc.call(self[i], self[i + 1]) == 1          # Spaceship Operator (<=>) Return Values:
                    self[i], self[i + 1] = self[i + 1], self[i] # -1 if a < b
                    sorted = false                              # 0 if a == b
                end                                             # 1 if a > b
            end
        end
